...
import Shelter from "../domain/models/Shelter/Shelter";
...
export default class ShelterController {
   ...
   async createShelter(
      ...
      const { name, password, email, phone, address: addressDTO } = req.body as CreateShelterDTO;
      const address = new AddressEntity(addressDTO.state, addressDTO.city);

      const shelter = new CreateShelterDTO(
         name, 
         password, 
         email, 
         phone, 
      ...
      const entityCreated = await this.repository.createShelter(ShelterMapper.toEntity(shelter));

      if (entityCreated) {
         const model = ShelterMapper.toModel(entityCreated);
         res.status(HttpStatusCode.CREATED).json(new ResponseAPI("Shelter created", instanceToPlain(ShelterMapper.toResponse(model))));
      }
   }
   ...
   async addPet(
      ...
      if (entity) {
         res.status(HttpStatusCode.OK).json(new ResponseAPI("Pet added to shelter"));
      } else {
   ...
   async getShelter(
      ...
      if (entity) {
         ...
         res.status(HttpStatusCode.OK).json(new ResponseAPI("Shelter found", instanceToPlain(ShelterMapper.toResponse(model))));
      } else {
   ...
   async getAllShelters(
      ...
      if (shelters.length == 0) {
         res.status(HttpStatusCode.NO_CONTENT).json(new ResponseAPI("No shelters found"));
         return;
      }
   ...
   async updateShelter(
      ...
      const entityUpdated = await this.repository.updateShelter(id, ShelterMapper.toEntity(shelter));

      if (entityUpdated) {
         const model = ShelterMapper.toModel(entityUpdated);
         res.status(HttpStatusCode.OK).json(new ResponseAPI("Shelter updated", instanceToPlain(ShelterMapper.toResponse(model))));
      } else {
         throw new NotFound("Shelter not found");
      }
   }
